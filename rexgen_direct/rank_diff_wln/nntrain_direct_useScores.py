...
'''
Script for training the candidate ranker model
'''
...
prediction = tf.argmax(score, 0)

_lr = tf.placeholder(tf.float32, [])
optimizer = tf.train.AdamOptimizer(learning_rate=_lr)

trainable_variables = tf.trainable_variables()
param_norm = tf.global_norm(trainable_variables)

gradients_and_varients = optimizer.compute_gradients(loss, trainable_variables)
gradients, var = zip(*gradients_and_varients)
gradient_norm = tf.global_norm(gradients)
new_gradients, _ = tf.clip_by_global_norm(gradients, max_norm)

# Create gradiant accumulation operations to smooth training
# i.e., only update after several minibatches
accum_grads = [tf.Variable(tf.zeros(v.get_shape().as_list()), trainable=False) for v in trainable_variables]
zero_ops = [v.assign(tf.zeros(v.get_shape().as_list())) for v in accum_grads]
accum_ops = [accum_grads[i].assign_add(grad) for i, grad in enumerate(new_gradients)]

gradients_and_varients = zip(accum_grads, var)
backprop = optimizer.apply_gradients(gradients_and_varients)
...
iterate, sum_acc, sum_err, sum_gnorm = 0, 0.0, 0.0, 0.0
lr = 0.001
...
try:
    while not coord.should_stop():
        ...
        for i in range(batch_size):
            ans = session.run(accum_ops + [prediction])
            if ans[-1] != 0: 
                sum_err += 1.0

        # Apply backprop after seeing batch_size examples
        _, pnorm, gnorm = session.run([backprop, param_norm, gradient_norm], feed_dict={_lr:lr})
        sum_gnorm += gnorm

        if iterate % 20 == 0 and iterate > 0:
            print("After %d, Training Error: %.4f, Parameter Norm: %.2f, Gradient Norm: %.2f" % (iterate, sum_err / 20, pnorm, sum_gnorm / 20 * batch_size))
            sys.stdout.flush()
            sum_err, sum_gnorm = 0.0, 0.0
        if iterate % 500== 0 and iterate > 0:
            saver.save(session, opts.save_path + "/model.ckpt-%d" % iterate)
            lr *= 0.98
            print("Learning Rate: %.6f" % lr)

        if iterate % 6000000 == 0 and iterate > 0:
            coord.request_stop()
