...
# whether to allow reagents/solvents to contribute atoms
...
try:
    while not coord.should_stop():
        ...
        if it % 5 == 0:
            ...
            sys.stderr.write('After seeing {} samples, accuracy@TopK={}: {:.3f}, acc@{}: {:.3f}, acc@{}: {:.3f}, acc@{}: {:.3f}, acc@{}: {:.3f}\n'.format(
    ...
    if tot_samples > 0:
        sys.stderr.write(
            'After seeing {} samples, accuracy@TopK={}: {:.3f}, acc@{}: {:.3f}, acc@{}: {:.3f}, acc@{}: {:.3f}, acc@{}: {:.3f}\n'.format(
                tot_samples, NK, accNK/tot_samples, NK0, accNK0/tot_samples, NK1, accNK1/tot_samples, NK2, accNK2/tot_samples, NK3, accNK3/tot_samples,
        ))
        sys.stderr.flush()
    coord.request_stop()
