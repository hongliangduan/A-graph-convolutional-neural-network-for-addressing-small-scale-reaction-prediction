...
parser.add_option("-b", "--batch", dest="batch_size", default=20)  # batch size is 20.
...
if opts.rich_feat:
    ...
    from rexgen_direct.core_wln_global.mol_graph import atom_feature_dimension as adim, bond_feature_dimension as bdim, max_nb, smiles2graph_list as _s2g
# There are two files: mol_graph and mol_graph_rich.
...
_binary = tf.placeholder(tf.float32, [batch_size, None, None, binary_feature_dimension])
...
binary.set_shape([batch_size, None, None, binary_feature_dimension])
...
# Calculate attention scores for each pair of atoms
...
pair_hidden = linearND(atom_pair, hidden_size, scope="atom_feature", init_bias=None) + \
              linearND(binary, hidden_size, scope="bin_feature", init_bias=None) + \
              linearND(att_pair, hidden_size, scope="ctx_feature")
...
bone_mask = tf.to_float(tf.equal(label, INVALID_BOND)) * 10000
_, topk = tf.nn.top_k(score - bone_mask, k=NK)
...
parameters_norm = tf.global_norm(tf.trainable_variables())
...
def read_data(path, coord):
    ...
    with open(path, 'r') as file:
        for line in file:
            reaction, edit = line.strip("\r\n ").split()
            c = count(reaction)
            for i in range(len(bucket_size)):
                if c <= bucket_size[i]:
                    buckets[i].append((reaction, edit))
                    break
...
iteration, sum_accuracy, sum_err, sum_gnorm = 0, 0.0, 0.0, 0.0
# lr = 0.001
lr = 0.001
...
try:
    while not coord.should_stop():
        ...
        # new_checkpoint = tf.train.latest_checkpoint(opts.save_path)
        # saver.restore(session, new_checkpoint)
        ...
        _, cur_topk, pnorm, gnorm = session.run([backprop, topk, parameters_norm, grad_norm], feed_dict={_lr:lr})
        ...
        for i in range(batch_size):
            ...
            for j in range(NK0):
                if cur_topk[i,j] in sp_label[i]:
                    pre += 1
            if len(sp_label[i]) == pre: sum_accuracy += 1
        sum_gnorm += gnorm

        if iteration % 50 == 0:
            print("Accuracy@10: %.4f, Acc@20: %.4f, Param Norm: %.2f, Grad Norm: %.2f"
                  % (sum_accuracy / (50 * batch_size), sum_err / (50 * batch_size), pnorm, sum_gnorm / 50))
            # print(iteration)
            

            sys.stdout.flush()
            sum_accuracy, sum_err, sum_gnorm = 0.0, 0.0, 0.0


        if iteration % 500 == 0:

            saver.save(session, opts.save_path + "/model.ckpt", global_step=iteration)
            print(iteration)
            print("Model Saved!")
